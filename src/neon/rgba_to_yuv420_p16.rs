//! NEON RGBA/RGB to YUV420 at precision 16 with split green coefficient.
//!
//! The BT.601 16-bit `yg = 33059` exceeds `i16::MAX` (32767), preventing
//! direct use of `vqdmlal_laneq_s16` which packs coefficients as i16. We
//! decompose `yg = yg_a + yg_b` where both halves fit i16, then use
//! non-doubling `vmlal_laneq_s16` with four MAC calls for the Y channel
//! (yr, yg_a, yg_b, yb). UV coefficients all fit i16 and use three MACs.
...
#[inline(always)]
unsafe fn encode_16_part_p16_420<const ORIGIN_CHANNELS: u8>(
    rgba0: &[u8],
    rgba1: &[u8],
    y_plane0: &mut [u8],
    y_plane1: &mut [u8],
    ...
    // Split each int16x8 into two int16x4 halves for vmlal (multiply-accumulate long)
    let r_h_low0 = vget_low_s16(r_high0);
    let g_h_low0 = vget_low_s16(g_high0);
    let b_h_low0 = vget_low_s16(b_high0);

    // Pixels 12-15: four MAC calls for split yg (yr + yg_a + yg_b + yb)
    // vmlal_high uses the high 4 lanes of the int16x8 input
    ...
    // Pixels 8-11: same four MACs using int16x4 (low half)
    // vmlal uses int16x4 (narrower form)
...
pub(crate) unsafe fn neon_rgba_to_yuv420_p16<const ORIGIN_CHANNELS: u8>(
    ...
    // Bias includes range offset and rounding constant for the final right-shift
    ...
    // v_weights_b: [0, yg_b, 0, 0, 0, 0, 0, 0]
    let weights_b_arr: [i16; 8] = [0, yg_b, 0, 0, 0, 0, 0, 0];
    ...
    while cx + 16 <= width {
        encode_16_part_p16_420::<ORIGIN_CHANNELS>(
